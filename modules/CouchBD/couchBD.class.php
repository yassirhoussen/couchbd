...
class CouchDB {
	...
	/**
	 * Simple construct. we send as the first parameter the couchServer url such as http://127.0.0.1:5984/
	 * The second parameter is the database to use. if not set, we use the default couchBD database, '_users'.
	 * 
	 * @param string $dns 
	 * @param string $tablename, default value = "_users"
	 */
	public function __construct($dns = NULL, $couchViewUrl = NULL, $tablename = "_users") {
		if (isset($dns))
			$this->client = new couchClient($dns, $tablename);
		else throw new Exception("Error Processing Request, couchDB server is not specified", 1);
		if(isset($couchViewUrl) ){
			$this->views =  require_once($couchViewUrl);
		}
		// else throw new Exception("Error Processing Request, couchDB view config url is not specified", 1);
		
 	}
	 
	/**
	* make couch use specfic database, in case the database is not set on couchDB instance 
	*
	...
	*/
 	...
 	/**
	* return a list of all databases, created on couchdb server
	...
	*/
 	...
 	public function query($name) {
 		$result = $this->client->getView('all', $name);
 		return $result;
 	}
